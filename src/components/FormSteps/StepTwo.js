...
export function StepTwo(props) {
    ...
    const handleClick = () => {
        if (props.state.fieldId !== undefined) {
            props.next();
        }
        else
            modalToggle();
    ...
    const renderItem = (res, index) => {
        return (
            <Item key={index} action href={`#${index}`} onClick={() => handleChange(res.fieldId)}>
                {res.name}
            </Item>
        );
    };
    ...
    return (
        <>
            <Title>Wybierz boisko dla którego chcesz przypisać mecz</Title>
            <StepWrapper>
                <List>
